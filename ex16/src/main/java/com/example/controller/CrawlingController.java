...
public class CrawlingController {
	...
	//주식
		...
		public HashMap<String, Object> stockJson()throws Exception{
			...
			Elements stock=doc.select("#_topItems1");
			ArrayList<HashMap<String, Object>> array=new ArrayList<HashMap<String, Object>>();
			for(Element st:stock.select("tr")) {
				HashMap<String, Object> s=new HashMap<String, Object>();
				s.put("title", st.select("a").text());
				s.put("no1", st.select("td:nth-child(2)").text());
				s.put("no2", st.select("td:nth-child(3)").text());
				s.put("no3", st.select("td:nth-child(4)").text());
				array.add(s);
			}		
			map.put("array",array);
			return map;
